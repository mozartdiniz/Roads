    ...
    xtag.register ('ro-list', {
        lifecycle: {
            created: function () {
                ...
                this.xtag.noDataTemplate = this.querySelector ('ro-nodata-item');
        ...
        methods: {
            ...
            parseNoDataItem: function () {

                var roNoDataItem = document.createElement ('ro-nodata-item');
                var key = this.xtag.noDataTemplate.getAttribute('i18nKey');
                roNoDataItem.innerHTML = Ro.templateEngine(key);

                this.appendChild(roNoDataItem)

            },
            parseDataItems: function () {
            ...
            parseList: function () {
                ...
                if (this.xtag.data.length) {
                    this.xtag.itemAction = this.action || this.xtag.item.getAttribute ('action');
                    this.parseDataItems();
                } else if (this.xtag.noDataTemplate) {
                    this.parseNoDataItem()
                }
